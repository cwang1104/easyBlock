...
func main() {
	...
	var ok bool
	var property int
	chain := block_chain.CreateBlockChain()
	property, _ = chain.FindUTXOs([]byte("start"))
	fmt.Println("balance of start: ", property)

	tempTx, ok = chain.CreateTransaction([]byte("start"), []byte("Krad"), 100)
	if ok {
	...
	tempTx, ok = chain.CreateTransaction([]byte("Krad"), []byte("Exia"), 200)
	if ok {
		txPool = append(txPool, tempTx)
	}
	...
	tempTx, ok = chain.CreateTransaction([]byte("start"), []byte("Exia"), 100)
	if ok {
		txPool = append(txPool, tempTx)
	}
	chain.Mine(txPool)
	...
	tempTx, ok = chain.CreateTransaction([]byte("Krad"), []byte("start"), 30)
	if ok {
		txPool = append(txPool, tempTx)
	}
